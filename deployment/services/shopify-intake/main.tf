...
locals {
  ...
  number_of_replicas = var.environment == "prod" ? 5 : 2
  container_port     = 8080
  service_port       = 9300
}
...
module "shopify-intake-topic" {
  source                 = "../../../modules/create_pubsub_topic"
  topic_name             = "shopify-intake-topic"
...
module "service" {
  ...
  container_command_args = [
    "spiffy/service/gateways/shopify_webhooks/__main__.py"
  ]
  ...
  env = [
    ...
    {
      name  = "SHOPIFY_APP_SPIFFY_AI_ANALYTICS_CLIENT_SECRET"
      value = data.google_secret_manager_secret_version.shopify-app-spiffy-ai-analytics-client-secret.secret_data
      sensitive = true
    }
  ]
