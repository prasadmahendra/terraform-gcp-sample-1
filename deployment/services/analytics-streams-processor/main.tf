...
resource "google_project_iam_custom_role" "iam_custom_role_for_service_cloudsql_access" {
  role_id     = "spiffy.shopifyStreamsSvcCloudSqlRole_${random_string.suffix.result}"
  project     = var.project_id
...
resource "google_project_iam_custom_role" "iam_custom_role_for_service_bq_access" {
  role_id     = "spiffy.shopifyDataStreamsSvcBigQueryRole_${random_string.suffix.result}"
  project     = var.project_id
  title       = "Role for ${var.service_name} service"
  description = "Terraform Managed - Role for ${var.service_name} service"
  permissions = concat(
...
resource "google_project_iam_custom_role" "iam_custom_role_for_service_bt_access" {
  role_id     = "spiffy.analyticsDataStreamsSvcBigTableRole_${random_string.suffix.result}"
  project     = var.project_id
  title       = "Role for ${var.service_name} service"
  description = "Terraform Managed - Role for ${var.service_name} service"
...
module "analytics-streams-service-topic-permissions" {
  source                       = "../../../modules/create_pubsub_sa_role"
  region                       = var.region
  project_id                   = var.project_id
  service_account_email        = google_service_account.service_account.email
  service_account_service_name = var.service_name
  topic_name                   = var.analytics_intake_topic_id
  deadletter_topic_name        = var.analytics_intake_topic_id
}

module "analytics-streams-processor" {
  source                      = "../../../modules/create_pubsub_topic_subscription"
  subscription_name           = "analytics-streams-processor"
  topic_id                    = var.analytics_intake_topic_id
  project_id                  = var.project_id
...
module "service" {
  ...
  container_command_args = [
    "spiffy/service/analytics/streams_processor/__main__.py"
  ]
  ...
  liveness_probe = {
    grpc = {
      service_name = "spiffy.service.analytics.analytics_intake.AnalyticsStreamsProcessor"
  ...
  env = [
    ...
    },
    {
      ...
      value = var.analytics_intake_topic_id
    },
    {
      name  = "ANALYTICS_STREAMS_PROCESSOR_SUBSCRIPTION_NAME"
      value = module.analytics-streams-processor.subscription_name
    },
    {
      name  = "ANALYTICS_STREAMS_PROCESSOR_SUBSCRIPTION_ID"
      value = module.analytics-streams-processor.subscription_id
    },
    {
      name  = "REDIS_HOST"
...
module "service_keda_horizontal_pod_autoscaler" {
  ...
  environment                     = var.environment
  max_replica_count               = var.environment == "prod" ? 12 : 2
  min_replica_count               = var.environment == "prod" ? 2 : 0 # cool down to zero when not in use
  ...
  subscription_id                 = module.analytics-streams-processor.subscription_id
