...
resource "google_project_iam_custom_role" "iam_custom_role_for_service_bt_access" {
  role_id     = "spiffy.shopifyDataStreamsSvcBigTableRole_${random_string.suffix.result}"
  project     = var.project_id
  title       = "Role for ${var.service_name} service"
  description = "Terraform Managed - Role for ${var.service_name} service"
...
module "shopify-streams-service-topic-permissions" {
  source                       = "../../../modules/create_pubsub_sa_role"
  region                       = var.region
  project_id                   = var.project_id
  service_account_email        = google_service_account.service_account.email
  service_account_service_name = var.service_name
  topic_name                   = var.shopify_intake_topic_id
  deadletter_topic_name        = var.shopify_intake_topic_id
}

module "shopify-streams-processor" {
  source                      = "../../../modules/create_pubsub_topic_subscription"
  subscription_name           = "shopify-streams-processor"
  topic_id                    = var.shopify_intake_topic_id
  project_id                  = var.project_id
...
module "service" {
  ...
  container_command_args = [
    "spiffy/service/cdp/shopify/streams_processor/__main__.py"
  ]
  ...
  liveness_probe = {
    grpc = {
      service_name = "spiffy.service.shopify.shopify_intake.ShopifyStreamsProcessor"
  ...
  env = [
    ...
    {
      name  = "SHOPIFY_PUBSUB_TOPIC_ID"
      value = var.shopify_intake_topic_id
    },
    {
      name  = "SHOPIFY_STREAMS_PROCESSOR_SUBSCRIPTION_NAME"
      value = module.shopify-streams-processor.subscription_name
    },
    {
      name  = "SHOPIFY_STREAMS_PROCESSOR_SUBSCRIPTION_ID"
      value = module.shopify-streams-processor.subscription_id
    },
    {
      name  = "REDIS_HOST"
    ...
    {
      name  = "SHOPIFY_BIGQUERY_TABLE_ID"
...
module "service_keda_horizontal_pod_autoscaler" {
  ...
  subscription_id                 = module.shopify-streams-processor.subscription_id
