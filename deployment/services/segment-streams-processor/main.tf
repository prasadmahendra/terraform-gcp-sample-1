...
resource "google_project_iam_custom_role" "iam_custom_role_for_service_bq_access" {
  role_id     = "spiffy.segmentStreamsSvcBigQueryRole_${random_string.suffix.result}"
  project     = var.project_id
...
resource "google_project_iam_custom_role" "iam_custom_role_for_service_bt_access" {
  role_id = "spiffy.segmentStreamsSvcBigTableRole_${random_string.suffix.result}"
  project = var.project_id
  title   = "Role for ${var.service_name} service"
  description = "Terraform Managed - Role for ${var.service_name} service"
...
resource "google_project_iam_custom_role" "iam_custom_role_for_service_cloudsql_access" {
  role_id     = "spiffy.segmentStreamsSvcCloudSqlRole_${random_string.suffix.result}"
  project     = var.project_id
  title       = "Role for ${var.service_name} service"
  description = "Terraform Managed - Role for ${var.service_name} service"
  permissions = concat(
...
module "segment-streams-service-topic-permissions" {
  source                       = "../../../modules/create_pubsub_sa_role"
  region                       = var.region
  project_id                   = var.project_id
  service_account_email        = google_service_account.service_account.email
  service_account_service_name = var.service_name
  topic_name                   = var.segment_intake_topic_id
  deadletter_topic_name        = var.segment_intake_topic_id
}

module "segment-streams-processor" {
  source                      = "../../../modules/create_pubsub_topic_subscription"
  subscription_name           = "segment-streams-processor"
  topic_id                    = var.segment_intake_topic_id
  project_id                  = var.project_id
...
module "service" {
  ...
  container_command_args = [
    "spiffy/service/cdp/segment/streams_processor/__main__.py"
  ]
  ...
  liveness_probe = {
    grpc = {
      service_name = "spiffy.service.segment.segment_intake.SegmentStreamsProcessor"
  ...
  env = [
    ...
    {
      name  = "SEGMENT_PUBSUB_TOPIC_ID"
      value = var.segment_intake_topic_id
    },
    {
      name  = "SEGMENT_STREAMS_PROCESSOR_SUBSCRIPTION_NAME"
      value = module.segment-streams-processor.subscription_name
    },
    {
      name  = "SEGMENT_STREAMS_PROCESSOR_SUBSCRIPTION_ID"
      value = module.segment-streams-processor.subscription_id
    },
    {
      name  = "REDIS_HOST"
    ...
    {
      name  = "SEGMENT_BIGQUERY_TABLE_ID"
...
module "service_keda_horizontal_pod_autoscaler" {
  ...
  subscription_id                 = module.segment-streams-processor.subscription_id
