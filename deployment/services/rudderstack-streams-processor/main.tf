...
resource "google_project_iam_custom_role" "iam_custom_role_for_service_bq_access" {
  role_id     = "spiffy.rudderStackStreamsSvcBigQueryRole_${random_string.suffix.result}"
  project     = var.project_id
...
resource "google_project_iam_custom_role" "iam_custom_role_for_service_bt_access" {
  role_id = "spiffy.rudderStackStreamsSvcBigTableRole_${random_string.suffix.result}"
  project = var.project_id
  title   = "Role for ${var.service_name} service"
  description = "Terraform Managed - Role for ${var.service_name} service"
...
resource "google_project_iam_custom_role" "iam_custom_role_for_service_cloudsql_access" {
  role_id     = "spiffy.rudderStackStreamsSvcCloudSqlRole_${random_string.suffix.result}"
  project     = var.project_id
  title       = "Role for ${var.service_name} service"
  description = "Terraform Managed - Role for ${var.service_name} service"
  permissions = concat(
...
module "simondata-streams-service-topic-permissions" {
  ...
  topic_name                   = var.rudderstack_intake_topic_id
  deadletter_topic_name        = var.rudderstack_intake_topic_id
}

module "rudderstack-streams-processor" {
  source                      = "../../../modules/create_pubsub_topic_subscription"
  subscription_name           = "rudderstack-streams-processor"
  topic_id                    = var.rudderstack_intake_topic_id
  project_id                  = var.project_id
...
module "service" {
  ...
  container_command_args = [
    "spiffy/service/cdp/rudderstack/streams_processor/__main__.py"
  ]
  ...
  liveness_probe = {
    grpc = {
      service_name = "spiffy.service.rudderstack.rudderstack_intake.RudderStackStreamsProcessor"
  ...
  env = [
    ...
    {
      name  = "RUDDERSTACK_PUBSUB_TOPIC_ID"
      value = var.rudderstack_intake_topic_id
    },
    {
      name  = "RUDDERSTACK_STREAMS_PROCESSOR_SUBSCRIPTION_NAME"
      value = module.rudderstack-streams-processor.subscription_name
    },
    {
      name  = "RUDDERSTACK_STREAMS_PROCESSOR_SUBSCRIPTION_ID"
      value = module.rudderstack-streams-processor.subscription_id
    },
    {
      name  = "REDIS_HOST"
    ...
    {
      name  = "RUDDERSTACK_BIGQUERY_TABLE_ID"
      value = var.persistence_bigquery_table_id
    },
    {
      name = "SEGMENT_BIGQUERY_TABLE_ID" # there is a typo in the code. fix the code before removing this!
      value = var.persistence_bigquery_table_id
    },
    {
...
module "service_keda_horizontal_pod_autoscaler" {
  ...
  subscription_id                 = module.rudderstack-streams-processor.subscription_id
