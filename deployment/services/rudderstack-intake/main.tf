...
module "rudderstack-intake-topic" {
  source                      = "../../../modules/create_pubsub_topic"
  topic_name                  = "rudderstack-intake-topic"
...
resource "google_project_iam_custom_role" "iam_custom_role_for_service_cloudsql_access" {
  role_id     = "spiffy.rudderStackIntakeSvcCloudSqlRole_${random_string.suffix.result}"
  project     = var.project_id
...
module "rudderstack-intake-topic-permissions" {
  source                       = "../../../modules/create_pubsub_sa_role"
  region                       = var.region
  project_id                   = var.project_id
  service_account_email        = google_service_account.service_account.email
  service_account_service_name = var.service_name
  topic_name                   = module.rudderstack-intake-topic.topic_name
...
module "service" {
  ...
  container_command_args = [
    "spiffy/service/gateways/rudderstack_webhooks/__main__.py"
  ]
  ...
  env = [
    ...
    },
    {
      ...
      value = module.rudderstack-intake-topic.topic_id
    },
    {
      name  = "REDIS_HOST"
