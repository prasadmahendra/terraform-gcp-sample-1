...
resource "google_project_iam_custom_role" "iam_custom_role_for_service_cloudsql_access" {
  role_id     = "spiffy.evaluationWorkerSvcCloudSqlRole_${random_string.suffix.result}"
  project     = var.project_id
...
resource "google_project_iam_custom_role" "iam_custom_role_for_service_gcs_access" {
  role_id     = "spiffy.evaluationWorkerSvcGcsRole_${random_string.suffix.result}"
  project     = var.project_id
  title       = "Role for ${var.service_name} service GCS access"
  description = "Terraform Managed - Role for ${var.service_name} service GCS access"
  permissions = [
    "storage.buckets.get",
    "storage.objects.get",
...
module "service" {
  ...
  container_command_args = [
    "-m",
    "spiffy.service.api.main.tools.run_evaluation_workers"
  ]
